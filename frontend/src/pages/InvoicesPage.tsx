...
export function InvoicesPage() {
  ...
  const pag = usePagination(filteredInvoices, 25, query);
  ...
  const allSelected =
    ...
    pag.pageItems.every((invoice) => selectedIds.includes(invoice.id));

  function toggleAllRows() {
    setSelectedIds((current) =>
      allSelected
        ? current.filter(
            (id) => !pag.pageItems.some((invoice) => invoice.id === id)
          )
        : Array.from(
            new Set([...current, ...pag.pageItems.map((invoice) => invoice.id)])
          )
    );
  }
